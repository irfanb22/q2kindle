...
// ── Mockup I: The Horizon ──────────────────────────────────────────────────
// A horizontal panorama — you pan left and right through a long rack of articles,
// like standing in front of a newsstand and scanning across the display.

const SCROLL_CARDS = 4; // How many cards to scroll per arrow click
...
export default function HorizonPage() {
  ...
  const toggleQueue = useCallback((id: string, e?: React.MouseEvent) => {
  ...
  const scrollBy = useCallback((dir: "left" | "right") => {
    ...
    const distance = cardWidth * SCROLL_CARDS;
    el.scrollBy({ left: dir === "right" ? distance : -distance, behavior: "smooth" });
  }, []);

  // Alternating tilt for physical feel
  const getCardRotation = (i: number) => (i % 2 === 0 ? 0.7 : -0.7);
    ...
    <div style={{ animation: "fadeUp 0.6s ease both" }} className="relative">
      ...
      <div className="relative">
        ...
        {canScrollLeft && (
          <button
            ...
            className="hidden lg:flex absolute top-1/2 -left-2 -translate-y-1/2 z-20 w-12 h-12 items-center justify-center rounded-full border transition-all duration-200 cursor-pointer"
            style={{ borderColor: "var(--color-border)", background: "var(--color-surface)", boxShadow: "0 4px 16px rgba(0,0,0,0.1)" }}
        ...
        {canScrollRight && (
          <button
            ...
            className="hidden lg:flex absolute top-1/2 -right-2 -translate-y-1/2 z-20 w-12 h-12 items-center justify-center rounded-full border transition-all duration-200 cursor-pointer"
            style={{ borderColor: "var(--color-border)", background: "var(--color-surface)", boxShadow: "0 4px 16px rgba(0,0,0,0.1)" }}
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--color-accent)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--color-border)"; }}
          >
        ...
        {canScrollLeft && (
          <div
            className="hidden lg:block absolute top-0 left-0 bottom-0 w-16 z-10 pointer-events-none"
            style={{ background: "linear-gradient(to right, var(--color-bg), transparent)" }}
          />
        )}
        {canScrollRight && (
          <div
            className="hidden lg:block absolute top-0 right-0 bottom-0 w-16 z-10 pointer-events-none"
        ...
        <div
          ref={scrollRef}
          className="horizon-scroll flex gap-4 overflow-x-auto pb-4"
          style={{
        ...
        >
          ...
          {filtered.map((article, i) => {
            const qs = queueStates[article.id] ?? "idle";
            ...
            return (
              <div
                key={article.id}
                className="horizon-card flex-shrink-0 relative rounded-xl overflow-hidden cursor-pointer"
                style={{
                  width: "240px",
                  minWidth: "240px",
                  aspectRatio: "2/3",
                  ...
                  animation: `cardFadeIn 0.4s ease both ${i * 0.03}s`,
                }}
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 40%, rgba(0,0,0,0.15) 60%, rgba(0,0,0,0.3) 100%)",
                  }}
                />

                {/* Source masthead (top) */}
                <div className="absolute top-0 left-0 right-0 p-3 flex items-center gap-1.5">
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: getFeedColor(article.feedId), flexShrink: 0 }} />
                ...
                <button
                  onClick={(e) => toggleQueue(article.id, e)}
                  className="absolute top-2.5 right-2.5 w-7 h-7 flex items-center justify-center rounded-full transition-all duration-300 cursor-pointer z-10"
                  style={{
                    background: qs === "queued" ? "var(--color-accent)" : qs === "animating" ? "var(--color-accent)" : "rgba(255,255,255,0.15)",
                ...
                >
                  ...
                  {qs === "animating" && (
                    <span className="text-[9px] font-bold text-white" style={{ animation: "cardFadeIn 0.3s ease both" }}>Q</span>
                  )}
                  {qs === "queued" && (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{ animation: "cardFadeIn 0.3s ease both" }}><path d="M5 13l4 4L19 7" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  )}
                </button>

                {/* Title + Read link (bottom) */}
      ...
      <div className="flex items-center justify-center mt-6">
        <span className="text-sm" style={{ color: "var(--color-text-dim)" }}>
          {filtered.length} articles — scroll to explore
        </span>
      </div>
